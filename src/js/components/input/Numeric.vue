...
export default {
  ...
  methods: {
    ...
    */
    formatValue () {
      this.amount = accounting.formatMoney(this.rawValue, {
    ...
    */
    convertToNumber () {
      if (this.rawValue === '0.00') {
        ...
        if (Math.floor(this.rawValue) === this.rawValue) {
  ...
  mounted () {
    ...
    if (this.rawValue) {
      this.processValue(this.formatToNumber(this.rawValue))
      this.formatValue(this.rawValue)
    }
